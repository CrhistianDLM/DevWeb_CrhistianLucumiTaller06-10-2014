$(document).ready(function () {
  ...
  $("#btnConsultar").click(function () {
    ...
    const getValue = (n) => {
      return document.getElementById(n).value.trim();
    };
    $.ajax({
      ...
      data: {
        txtNumero: getValue("txtNumero"),
        txtLetra: getValue("txtLetra"),
        txtInverso: getValue("txtInverso"),
        txtNInverso: getValue("txtNInverso"),
        txtTI: getValue("txtTI"),
      },
      type: "POST",
    ...
    $.each(data, function (key, val) {
      $("#BodyTable").append(
        "<tr>\
          ...
          val.usuario +
          "</th>\
										<th> <button type='button' class='btn btn-secondary' data-v='"+val.nombre+"' onclick='playNumber(event)'>Play</button>"+ 
...
function DecenasY(strSin, numUnidades) {
  if (numUnidades > 0)
    return (
      strSin + (activeLanguage == ESP ? " Y " : "") + Unidades(numUnidades)
    );
...
function resultadoTraduccion(result) {
  ...
  if (result.translation) divtraduccion.innerHTML = result.translation;
}
function playNumber(event) {
  meSpeak.speak(event.target.dataset.v);
}
